...
- [ ] Этот файл `back/TODO.md` не удаляется при генерации Nest (при конфликте — временно перенести или создать проект в подпапке и перенести файлы)
...
- [ ] В `package.json` dev-скрипт явно задаёт порт (Nest: `start:dev`)
...
- [ ] Префикс путей согласован с фронтом (см. абзац **«Префикс путей»** в [TODO.md](../TODO.md)):
  - [ ] **Вариант A (типично для Nest):** **`/api/projects`**, **`/api/tasks`**, **`/api/tasks/:id`** и т.д.
...
- [ ] В мастере/флагах: TypeScript и npm; проект как API (без лишнего UI).
- [ ] Откройте **`back/package.json`**: зависимости `@nestjs/*` — стабильные версии.

- [ ] В `package.json` задайте `start:dev` с портом (после правки файла вручную или через редактор):
...
- [ ] `GET /api/health` в контроллере — быстрый smoke endpoint для проверки, что API поднят

---

## 3. После выбора стека
