...
- **[front/TODO.md](front/TODO.md)** — Angular: раздел **«Окружение (до `ng new`)»** (= **этап 0** корневой карты ниже; отдельно от заголовков «Этап 1»…), затем подробные **этапы 1–14**; структура `src/app`, HTTP, signals, тесты и т.д.
...
Одна строка в `-m "..."`. Текст **после** `<тип>:` — **только на английском**: короткий **imperative** (как в Conventional Commits: _add_, _fix_, _update_, …) или очень краткое нейтральное описание; без кириллицы в сообщении коммита.
...
- **15** На вырост: ACL/RBAC, toasts, история, Nest + WebSocket и др. — опциональный post-MVP этап, см. [back/TODO.md](back/TODO.md) §5 «На вырост».
  - **Фронт-часть:** Этап 15 в [front/TODO.md](front/TODO.md) — toasts (Material), валидационные ошибки (mapping `class-validator`) и UI-gating по ролям на основе `GET /api/auth/me`.
  - **Первые подшаги (бекенд):** §5.1 **DTO + `class-validator`**, затем по приоритету БД или JWT.
