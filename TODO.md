...
- **[back/TODO.md](back/TODO.md)** — API: **§0** «Окружение и инструменты» (= **этап 0** корневой карты); по умолчанию **Next.js** (стабильная, Route Handlers); **json-server** — другой обычный вариант (REST из `db.json`), порты, CORS/proxy.
...
**Бэкенд по умолчанию** для учебного пути — **Next.js** последней стабильной версии (API в `back/`). Рядом в том же чеклисте — **json-server** как обычный вариант REST на файле без Next, см. [back/TODO.md](back/TODO.md).

Для агента: не удалять чужие `console.log` и комментарии; не читать `.env`; менять только относящееся к задаче. При добавлении шагов с командами в `front/TODO.md` / `back/TODO.md` — **оформлять по шаблону** из раздела ниже (команда + расшифровка для ученика). Предлагая текст коммита — формат **`T-<N> <тип>: …`** из раздела «Сообщения коммитов» (номер **N** подставляет ученик; описание после двоеточия — **только на английском**).
...
- [ ] **Команда:** `npm install json-server --save-dev`  
  - **Что делает:** скачивает пакет `json-server` в `node_modules`, добавляет зависимость в `package.json`, обновляет lockfile.  
  - **Где:** каталог `back/` (там лежит ваш `package.json`).  
  - **Успех:** в конце нет `npm ERR!`; в `package.json` появилась строка с `json-server`.  
  - **Частая ошибка:** `EACCES` при глобальной установке — не путать с локальной `-D` в проекте.
...
5. Остальное (формы, CDK, фильтры, тесты, SSR, i18n, PWA, a11y) — по оглавлению в [front/TODO.md](front/TODO.md).

**Выбор `back/`:** основной путь — **Next.js stable**, Route Handlers в `app/api/...`, отдельный порт (например `3001`), CORS или proxy. **json-server** — обычный вариант: тот же REST, данные в `db.json`, отдельный процесс на своём порту.
...
- **Next + Angular** в dev: две Node-зависимости; при странностях сверить версии Node/пакетов и не смешивать два `back` на одном порту.
...
- [x] ✅ **1** Каркас `front/` (routing, SCSS, standalone, strict TS; **CSR** до этапа 10, см. [front/TODO.md](front/TODO.md))
